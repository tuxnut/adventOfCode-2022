...
def compute_points(opponent_choice: str, own_choice: str) -> int:
    if (own_choice == ROCK and opponent_choice == SCISSORS) or (own_choice == PAPER and opponent_choice == ROCK) or (own_choice == SCISSORS and opponent_choice == PAPER):
...
def compute_round_score(round: str, problem_part: int) -> int:
    ...
    own_choice = (decrypt_own_choice_part_one(round[2]) if problem_part == 1
        else decrypt_own_choice_part_two(round[0], round[2])
    )
